...
#define OBJ_FILE                ( char * )"basketball.obj"
//#define VERTEX_SHADER_FILE      ( char * )"vertex.glsl"
//#define FRAGMENT_SHADER_FILE    ( char * )"fragment.glsl"

#define VERTEX_SHADER_FILE_FOR_TEXTURE      ( char * )"vertex_texture.glsl"
#define FRAGMENT_SHADER_FILE_FOR_TEXTURE    ( char * )"fragment_texture.glsl"

#define VERTEX_SHADER_FILE_FOR_MATERIAL      ( char * )"vertex_texture.glsl"
#define FRAGMENT_SHADER_FILE_FOR_MATERIAL    ( char * )"fragment_material.glsl"

//#define VERTEX_SHADER_FILE_FOR_TEXTURE      ( char * )"vertex.glsl"
//#define FRAGMENT_SHADER_FILE_FOR_TEXTURE    ( char * )"fragment.glsl"
//
//#define VERTEX_SHADER_FILE_FOR_MATERIAL      ( char * )"vertex.glsl"
//#define FRAGMENT_SHADER_FILE_FOR_MATERIAL    ( char * )"fragment.glsl"
...
nbd::vec3 location = { 0.0f, -1.0f, -3.9f };

float rotz = 0.0f, rotx = 110.0f;
...
//    glUniform1i( program->getUniformLocation(( char * )"DIFFUSE" ), 1 );
    ...
    for (int i = 0; i < obj->materialList.size(); i++) {
        Material *material = obj->materialList[i];
        material->bindTexture(obj->textureList);
        //        if (strstr(material->name, "Seats")) {
        if (material->hasTexture()) {
            material->program = Program::create(material->name, VERTEX_SHADER_FILE_FOR_TEXTURE, FRAGMENT_SHADER_FILE_FOR_TEXTURE, 1, 1, nbManager::programBindAttrLocation, NULL);
        }
        else {
            material->program = Program::create(material->name, VERTEX_SHADER_FILE_FOR_MATERIAL, FRAGMENT_SHADER_FILE_FOR_MATERIAL, 1, 1, nbManager::programBindAttrLocation, NULL);
        }
        
        material->materialdrawcallback = nbManager::materialDrawCallback;
    }
}
...
//            rotz = CLAMP( rotz, 0.0f, 180.0f );
    ...
    if( move_delta.z )
    {
        ...
        forward.x = c * move_delta.x - s * move_delta.y;
        forward.y = s * move_delta.x + c * move_delta.y;
...
//    }
    ...
    for (int i = 0; i < obj->meshList.size(); i++) {
        ...
        if (mesh->rigidBody) {
            ...
            if( strstr( mesh->name, "ball" ) ) {
                if (mesh->location.z < -6) {
                    mesh->location.x = 0.0;
                    mesh->location.y = 2.39;
                    mesh->location.z = -3.3;
                    
                    btVector3 position( mesh->location.x, mesh->location.y, mesh->location.z );
                    btTransform trans;
                    mesh->rigidBody->getMotionState()->getWorldTransform( trans );
                    trans.setOrigin( position );
                    mesh->rigidBody->clearForces();
                    mesh->rigidBody->setLinearVelocity( btVector3( 0, 0, 0) );
                    mesh->rigidBody->setAngularVelocity( btVector3( 0, 0, 0) );
                    mesh->rigidBody->setWorldTransform( trans );
                    
                    mesh->rigidBody->forceActivationState(ISLAND_SLEEPING);
                    
                    location = { 0.0f, -1.0f, -3.9f };
                    
                    rotz = 0.0f, rotx = 110.0f;
                    
                    
                    view_location = { 0.0f, 0.0f };
                    view_delta = { 0.0f, 0.0f };
                    
                    move_location = { 0.0f, 0.0f, 0.0f };
...
void nbManager::toucheEnded( float x, float y, unsigned int tap_count )
{
    ...
    if( sphere) {
        ...
        nbd::vec3 v = nbd::vec3((x - tapPoint.x) * 0.1, 5, - ((y - tapPoint.y) * 0.1));
        
        rotm.translate(v);
        
        nbd::vec4 xRotate = nbd::vec4(1, 0, 0, rotx);
//        v = v.rotate(xRotate);
        
//        rotm.rotate(xRotate);
        
        nbd::vec4 zRotate = nbd::vec4(0, 0, 1, rotz);
//        v = v.rotate(zRotate);
...
//                                                       v.z ) );
        ...
        sphere->rigidBody->setLinearVelocity(
                                             btVector3( rotm.m[ 3 ].x,
                                                       rotm.m[ 3 ].y,
                                                       rotm.m[ 3 ].z ) );
        
        
//        float velocityFactorY = ( y - tapPoint.y ) * 0.1f;
//        float velocityFactorX = ( x - tapPoint.x ) * 0.1f;
//        sphere->rigidBody->setLinearVelocity(
//                                             btVector3( velocityFactorY,
//                                                       0.0f,
//                                                       velocityFactorX ) );
...
void nbManager::programDrawCallback(void *ptr)
{
    ...
    for (int i = 0; i < program->uniformList.size(); i++) {
        ...
        if( !strcmp( uniform.name, "DIFFUSE" ) ) {
            ...
            nbd::vec3 position    = { 4.0f, 1.0f, 5.9f };
            nbd::vec3 eyeposition = { 0.0f,  0.0f,  0.0f };
            
//            eyeposition = position.multiply(nbManager::matrixMgr->getModelViewMatrix());
            
            nbd::mat4 &m = nbManager::matrixMgr->getModelViewMatrix();
            matrixMgr->popMatrix();
            eyeposition = position.multiply(nbManager::matrixMgr->getModelViewMatrix());
            matrixMgr->pushMatrix(m);
            glLogVerbose("glUniform3fv : LIGHTPOSITION");
            glUniform3fv( uniform.location,
                         1,
                         ( float * )&position );
