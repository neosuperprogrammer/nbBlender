...
namespace nbd {
    ...
    Texture2 *Texture2::create(char *name, char *filename, unsigned char relative_path, unsigned int flags, unsigned char filter, float anisotropic_filter)
    {
        Texture2 *texture = new Texture2(name);
    ...
    void Texture2::build(unsigned int  flags,
    ...
    void Texture2::load(Memory *memory)
    {
        char ext[MAX_CHAR] = {""};
     ...
     */
    void png_memory_read_callback2(png_structp structp, png_bytep bytep, png_size_t size)
    {
        Memory *m = (Memory *)png_get_io_ptr(structp);
        m->read(bytep, (unsigned int)size);
    }
    
    void Texture2::load_png(Memory *memory)
    {
        png_structp structp;
        ...
        png_set_read_fn( structp, ( png_voidp * )memory, png_memory_read_callback2 );
    ...
    void Texture2::load_pvr(Memory *memory)
    {
    ...
    void Texture2::convert_16_bits(unsigned char use_5551)
    {
        unsigned int i = 0,
    ...
    void Texture2::generate_id(unsigned int flags, unsigned char filter, float anisotropic_filter)
    {
        if (this->tid) {
            delete_id();
        }
    ...
    void Texture2::free_texel_array()
    {
        if( this->texel_array )
        {
            free( this->texel_array );
    ...
    void Texture2::draw()
    {
        glLogVerbose("Texture2::draw > glBindTexture");
        glBindTexture( this->target,
                      this->tid );
    }
    
    void Texture2::scale(unsigned int width, unsigned int height)
    {
        unsigned int i = 0,
        j,
        offset,
