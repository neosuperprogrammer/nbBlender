...
namespace nbd {
    ...
    void getPathFromFilePath(char *path, char *filepath)
    {
        char *t = NULL;
    ...
    void getFileNameFromFilePath(char *filename, char *filepath)
    {
        char *t = NULL;
    ...
    void create_direction_vector( vec3 *dst, vec3 *up_axis, float rotx, float roty, float rotz )
    {
        ...
        *up_axis = up_axis->invert();
        
        //	vec3_multiply_mat4( dst, up_axis, &m );
        
        *dst = up_axis->multiply(m);
    }
