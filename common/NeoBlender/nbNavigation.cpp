...
//                             objmesh->n_objvertexdata,
    ...
    unsigned char NAVIGATION_get_path( NAVIGATION *navigation, NAVIGATIONPATH *navigationpath, NAVIGATIONPATHDATA *navigationpathdata )
    {
        ...
        start_location = start_location.toRecast();
//        vec3_to_recast( &start_location );
        
        end_location = end_location.toRecast();
...
//        vec3_to_recast( &end_location );
        ...
        if( navigationpath->poly_count )
        {
            ...
            if( navigationpathdata->path_point_count )
            {
                ...
                {
                    navigationpathdata->path_point_array[ i ] = navigationpathdata->path_point_array[ i ].recastTo();
//                    recast_to_vec3( &navigationpathdata->path_point_array[ i ] );
     ...
     */
    void NAVIGATION_draw( NAVIGATION *navigation, MatrixMgr &matrixMgr )
    {
        if( !navigation->program )
        {
            

            Program *program = new Program(navigation->name);
            program->vertexShader = new Shader(navigation->name, GL_VERTEX_SHADER);
            program->vertexShader->compile("uniform highp mat4 MODELVIEWPROJECTIONMATRIX;"
                                           "attribute highp vec3 POSITION;"
                                           "void main( void ) {"
                                           "gl_Position = MODELVIEWPROJECTIONMATRIX * vec4( POSITION, 1.0 ); }",
                                           0 );
            
            program->fragmentShader = new Shader(navigation->name, GL_FRAGMENT_SHADER);
            program->fragmentShader->compile("void main( void ) {"
...
//            PROGRAM_link( navigation->program, 0 );	
        ...
        char vertex_attribute = navigation->program->getVertexAttribLocation(( char * )"POSITION" );
...
//        PROGRAM_draw( navigation->program );
        ...
        glUniformMatrix4fv( navigation->program ->getUniformLocation(( char * )"MODELVIEWPROJECTIONMATRIX"),
                           1,
                           GL_FALSE,
