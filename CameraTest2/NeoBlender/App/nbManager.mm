...
void nbManager::loadObject(void)
{
    ...
    player->rigidBody->setFriction( 10.0f );
    
    memcpy( &eye, &player->location, sizeof( nbd::vec3 ) );
...
void nbManager::draw(void)
{
    ...
    if( move_delta.z )
    {
        ...
        player->rigidBody->setAngularVelocity( btVector3(  direction.y * ( move_delta.z * 6.7f ),
                                                           -direction.x * ( move_delta.z * 6.7f ),
                                                           0.0f ) );
        
        player->rigidBody->setActivationState( ACTIVE_TAG );
    }
    
    
    next_eye.x = player->location.x + ::distance * cosf( rotx * DEG_TO_RAD ) * sinf( rotz * DEG_TO_RAD );
    ...
    ClosestNotMeRayResultCallback back_ray(player->rigidBody,
                                           p1,
                                           p2);
    ...
    if (doubleTapped) {
                                 ...
                                 &location.z  ))
        {
            ...
            btVector3 ray_to( location.x + eye.x,
                             location.y + eye.y,
                             location.z + eye.z );
