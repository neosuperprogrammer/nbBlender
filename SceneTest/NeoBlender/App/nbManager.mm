...
#define OBJ_FILE                ( char * )"scene2.obj"
#define VERTEX_SHADER_FILE      ( char * )"vertex_test3.glsl"
#define FRAGMENT_SHADER_FILE    ( char * )"fragment_test3.glsl"


#define VERTEX_SHADER_FILE_FOR_TEXTURE      ( char * )"vertex_test3.glsl"
#define FRAGMENT_SHADER_FILE_FOR_TEXTURE    ( char * )"fragment_test3.glsl"

#define VERTEX_SHADER_FILE_FOR_MATERIAL      ( char * )"vertex_test3.glsl"
...
{
    nbObj.justLoadObjFile(OBJ_FILE, 1);
    
    for (int i = 0; i < nbObj.meshList.size(); i++) {
        Mesh *mesh = nbObj.meshList[i];
        
        mesh->updateBoundForAllObject(&nbObj.vertexMgr);
        mesh->buildVboVertex(&nbObj.vertexMgr);
        mesh->buildVboVertexIndex();
        mesh->buildVao();
        
        mesh->freeVertexData();
    }
    
    nbObj.freeVertextData();
    
    for (int i = 0; i < nbObj.textureList.size(); i++) {
        Texture *texture = nbObj.textureList[i];
        texture->build(TEXTURE_MIPMAP, TEXTURE_FILTER_2X, 0.0f);
    }
    
    for (int i = 0; i < nbObj.materialList.size(); i++) {
        Material *material = nbObj.materialList[i];
        material->bindTexture(nbObj.textureList);
...
void nbManager::draw(void)
{
    ...
    // mushroom.obj
    nbd::vec3 e = {  -127.0f, 101.0f, 77.0f },
    c = {  -107.0f, -55.0f, 0.0f },
    u = {  0.0f,  0.0f, 1.0f  };
...
//        if( mesh->material == nil || mesh->material->dissolve == 1.0f ) {
            ...
            if (strstr(mesh->name, "Mainsail") || strstr(mesh->name, "Jib")) {
//                glEnable( GL_BLEND );
